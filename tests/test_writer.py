...
@pytest.fixture
def writer_cli_args(tmpdir_cwd):
...
@pytest.fixture
def writer_context(writer_cli_args):
    @contextlib.contextmanager
    def func():
        with cli.make_context("pip-compile", writer_cli_args) as ctx:
            writer = OutputWriter(
...
@pytest.fixture
def writer(tmpdir_cwd, writer_context):
...
def test_write_header_group_opt(writer_cli_args, writer_context, groupspec):
    writer_cli_args.clear()
    writer_cli_args.extend(["--dry-run", "--group", groupspec])
