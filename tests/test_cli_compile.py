...
def test_default_pip_conf_read(pip_with_index_conf, runner):
    ...
    with open("requirements.in", "w"):
        pass
    out = runner.invoke(cli, ["-v", "--allow-unsafe"])
...
def test_command_line_overrides_pip_conf(pip_with_index_conf, runner):
    ...
    with open("requirements.in", "w"):
        pass
    out = runner.invoke(cli, ["-v", "-i", "http://override.com", "--allow-unsafe"])
...
def test_command_line_setuptools_read(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["--no-emit-find-links", "--allow-unsafe"])
    ...
    assert out.stderr == dedent(
        ...
        # To update, run:
        #
        #    pip-compile --allow-unsafe --no-emit-find-links
        #
        small-fake-a==0.1
...
def test_command_line_setuptools_output_file(
    ...
    out = runner.invoke(cli, ["--allow-unsafe"] + options)
...
def test_find_links_option(runner):
    ...
    out = runner.invoke(cli, ["-v", "-f", "./libs1", "-f", "./libs2", "--allow-unsafe"])
...
def test_all_no_emit_options(runner, options):
    with open("requirements.in", "w"):
        pass
    out = runner.invoke(cli, ["--no-header", "--allow-unsafe"] + options)
...
@pytest.mark.parametrize(
    "options",
    (
        ["--index", "--emit-index-url"],
        ["--index", "--no-emit-index-url"],
        ["--no-index", "--emit-index-url"],
        ["--no-index", "--no-emit-index-url"],
    ),
)
...
def test_realistic_complex_sub_dependencies(runner):
    ...
    out = runner.invoke(cli, ["-n", "--rebuild", "-f", wheels_dir, "--allow-unsafe"])
...
def test_run_as_module_compile():
    ...
    status, output = invoke(
        [sys.executable, "-m", "piptools", "compile", "--help", "--allow-unsafe"]
    )
...
def test_editable_package(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["-n", "--allow-unsafe"])
...
def test_editable_package_vcs(runner):
    ...
    out = runner.invoke(cli, ["-n", "--rebuild", "--allow-unsafe"])
    assert out.exit_code == 0
...
def test_locally_available_editable_package_is_not_archived_in_cache_dir(
    ...
    out = runner.invoke(
        cli, ["-n", "--rebuild", "--cache-dir", str(cache_dir), "--allow-unsafe"]
    )
...
@pytest.mark.parametrize("cli_options", (["--generate-hashes"], []))
@pytest.mark.network
def test_url_package(runner, line, dependency, cli_options):
    with open("requirements.in", "w") as req_in:
        req_in.write(line)
    out = runner.invoke(cli, ["-n", "--rebuild", "--allow-unsafe"] + cli_options)
    assert out.exit_code == 0
...
def test_local_url_package(
    pip_conf, runner, line, dependency, rewritten_line, cli_options
):
...
def test_input_file_without_extension(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["requirements", "--allow-unsafe"])
...
def test_upgrade_packages_option(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["--no-annotate", "-P", "small-fake-b", "--allow-unsafe"])
...
def test_upgrade_packages_option_irrelevant(pip_conf, runner):
    ...
    out = runner.invoke(
        cli, ["--no-annotate", "--upgrade-package", "small-fake-b", "--allow-unsafe"]
    )
...
def test_upgrade_packages_version_option(
    ...
    out = runner.invoke(
        cli, ["--no-annotate", "--upgrade-package", upgraded_package, "--allow-unsafe"]
    )
...
def test_upgrade_packages_version_option_no_existing_file(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["-P", "small-fake-b==0.2", "--allow-unsafe"])
...
def test_upgrade_packages_version_option_and_upgrade(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["--upgrade", "-P", "small-fake-b==0.1", "--allow-unsafe"])
...
def test_quiet_option(runner):
    with open("requirements", "w"):
        pass
    out = runner.invoke(cli, ["--quiet", "-n", "requirements", "--allow-unsafe"])
...
def test_dry_run_noisy_option(runner):
    with open("requirements", "w"):
        pass
    out = runner.invoke(cli, ["--dry-run", "requirements", "--allow-unsafe"])
...
def test_dry_run_quiet_option(runner):
    with open("requirements", "w"):
        pass
    out = runner.invoke(cli, ["--dry-run", "--quiet", "requirements", "--allow-unsafe"])
...
def test_generate_hashes_with_editable(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["--no-annotate", "--generate-hashes", "--allow-unsafe"])
    expected = (
        "-e {}\n"
        "small-fake-a==0.1 \\\n"
...
def test_generate_hashes_verbose(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["--generate-hashes", "-v", "--allow-unsafe"])
...
def test_generate_hashes_with_annotations(runner):
    ...
    out = runner.invoke(cli, ["--generate-hashes", "--allow-unsafe"])
    assert out.stderr == dedent(
        """\
        #
        # This file is autogenerated by pip-compile
...
def test_filter_pip_markers(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["-n", "requirements", "--allow-unsafe"])
...
def test_no_candidates_pre(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["-n", "requirements", "--pre", "--allow-unsafe"])
...
def test_not_specified_input_file(runner):
    ...
    """
    out = runner.invoke(cli, ["--allow-unsafe"])
...
def test_stdin(pip_conf, runner):
    ...
    assert out.stderr == dedent(
        ...
        # To update, run:
        #
        #    pip-compile --allow-unsafe --no-emit-find-links --output-file=requirements.txt -
        #
        small-fake-a==0.1
            # via -r -
...
def test_multiple_input_files_without_output_file(runner):
    ...
    out = runner.invoke(cli, ["src_file1.in", "src_file2.in", "--allow-unsafe"])
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            # To update, run:
            #
            #    pip-compile --allow-unsafe --no-annotate --no-emit-find-links
            #
            small-fake-a==0.1
            small-fake-with-deps==0.1
            Dry-run, so nothing updated.
...
def test_annotate_option(pip_conf, runner, option, expected):
    ...
    out = runner.invoke(cli, [option, "-n", "--no-emit-find-links", "--allow-unsafe"])
...
@pytest.mark.parametrize(
    ("option", "expected"),
    (("--allow-unsafe", "small-fake-a==0.1"), ("--no-allow-unsafe", "# small-fake-a")),
)
def test_allow_unsafe_option(recwarn, pip_conf, monkeypatch, runner, option, expected):
    """
...
def test_allow_unsafe_deprecation_warning(pip_conf, monkeypatch, runner):
    monkeypatch.setattr("piptools.resolver.UNSAFE_PACKAGES", {"small-fake-a"})
    with open("requirements.in", "w") as req_in:
        req_in.write(path_to_url(os.path.join(PACKAGES_PATH, "small_fake_with_deps")))

    msg = (
        "In future versions of pip-tools, the --allow-unsafe behavior will be "
        "used by default and the option will be removed. It is recommended to "
        "pass the argument now to adapt projects to the upcoming change. To "
        "suppress this warning pass either --allow-unsafe or "
        "--no-allow-unsafe."
    )
    ...
    assert out.exit_code == 0
    # Uses the --no-allow-unsafe behavior.
    print(out.stderr)
    assert out.stderr == dedent(
        """\
        #
        ...
        {}/packages/small_fake_with_deps
            # via -r requirements.in

        # The following packages are considered to be unsafe in a requirements file:
        # small-fake-a
        """.format(
            path_to_url(TEST_DATA_PATH)
        )
    )
...
def test_cert_option(parse_requirements, runner, option, attr, expected):
    ...
    runner.invoke(cli, [option, expected, "--allow-unsafe"])
...
def test_build_isolation_option(parse_requirements, runner, option, expected):
    ...
    runner.invoke(cli, [option, "--allow-unsafe"])
...
def test_forwarded_args(PyPIRepository, runner):
    ...
    cli_args = ("--no-annotate", "--generate-hashes", "--allow-unsafe")
...
@pytest.mark.parametrize(
    ("cli_options", "infile_option", "expected_package"),
    (
        # no --pre pip-compile should resolve to the last stable version
        ([], False, "small-fake-a==0.2"),
        # pip-compile --pre should resolve to the last pre-released version
        (["-p"], False, "small-fake-a==0.3b1"),
        ([], True, "small-fake-a==0.3b1"),
        (["-p"], True, "small-fake-a==0.3b1"),
    ),
)
def test_pre_option(pip_conf, runner, cli_options, infile_option, expected_package):
    """
    ...
    out = runner.invoke(cli, ["--no-annotate", "-n", "--allow-unsafe"] + cli_options)
...
def test_dry_run_option(pip_conf, runner, add_options):
    ...
    out = runner.invoke(
        cli, ["--no-annotate", "--dry-run", "--allow-unsafe"] + add_options
    )
...
def test_empty_input_file_no_header(runner, empty_input_pkg, prior_output_pkg):
    ...
    runner.invoke(cli, ["--no-header", "requirements.in", "--allow-unsafe"])
...
def test_upgrade_package_doesnt_remove_annotation(pip_conf, runner):
    ...
    runner.invoke(cli, ["--allow-unsafe"])
    ...
    runner.invoke(cli, ["-P", "small-fake-a", "--no-emit-find-links", "--allow-unsafe"])
    with open("requirements.txt", "r") as req_txt:
...
def test_unreachable_index_urls(runner, cli_options, expected_message):
    ...
    out = runner.invoke(cli, ["--allow-unsafe"] + cli_options)
...
def test_remove_outdated_options(runner, input_opts, output_opts):
    ...
    out = runner.invoke(cli, ["--no-header", "--allow-unsafe"])
...
def test_sub_dependencies_with_constraints(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["--no-annotate", "--allow-unsafe"])
...
def test_preserve_compiled_prerelease_version(pip_conf, runner):
    ...
    out = runner.invoke(cli, ["--no-annotate", "--no-header", "--allow-unsafe"])
...
def test_prefer_binary_dist(
    ...
    out = runner.invoke(
        cli,
        ["--no-annotate", "--find-links", str(dists_dir), "--allow-unsafe"],
...
def test_duplicate_reqs_combined(
    ...
    out = runner.invoke(cli, ["--find-links", str(dists_dir), "--allow-unsafe"])
