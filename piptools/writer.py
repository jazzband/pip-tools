...
class OutputWriter(object):
    ...
    def write_header(self):
        if self.emit_header:
            filenames = self.string_of_input_filenames()
            yield comment('#')
            yield comment('# This file is autogenerated by pip-compile')
            yield comment('# Make changes in {}, then run this to update:'.format(filenames))
            yield comment('#')
            args = ''
            ...
            if not self.annotate:
                args += '--no-annotate '
            yield comment('#    pip-compile {args}{filenames}'.format(
                          args=args,
                          filenames=filenames))
            yield comment('#')

    def string_of_input_filenames(self):
        filenames = ''
