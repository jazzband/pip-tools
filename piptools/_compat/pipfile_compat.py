...
from pip._internal.req.req_file import handle_line
...
class ValuesFactory:
    ...
    def build_values(cls, values_dict, defaults=None):
        values = cls.option_parser.get_default_values() if defaults is None else Values(defaults)
...
def _handle_options(line, filename=None, lineno=None, finder=None, options=None, session=None):
    key, value = line
    ...
    if key == 'sources':
        if len(value) == 0:
            ...
            trusted_hosts = [urlparse(source['url']).netloc for source in value if not source.get('verify_ssl', True)]
            opts = dict(index_url=index_url, extra_index_urls=extra_urls, trusted_hosts=trusted_hosts)
...
def _parse_requirement(line, filename=None, lineno=None):
    ...
    if isinstance(values, str):
        ...
        opts = dict(
            editables=["file://{}".format(os.path.abspath(values['path']))]
        )

    return ParsedLine(filename, lineno, args, Values(opts), False)


def _parse_pipfile(filename,
                   session,
                   finder=None,
                   options=None,
                   constraint=False,
...
def parse_pipfile(
    ...
    for parsed_req in _parse_pipfile(filename, session, finder, options, constraint):
